...
skip_download = parameters.skip_download
...
def _get_target_url_dict(start_year, end_year, already_scraped_pdf_files):
    ...
    print('len(already_scraped_pdf_files)=', len(already_scraped_pdf_files))
...
def main():
    ...
    for one_file in pdf_filepaths:
        ...
        txt_filepath = os.path.join(txt_dir, filename_only + ".txt")
        try:
            if not os.path.exists(txt_filepath):
                _whole_txt = pdf2txt(one_file, txt_dir, wo_special_char_regex)
                save_txt(os.path.join(txt_dir, txt_filepath), _whole_txt)
            else:
                _whole_txt = read_txt(txt_filepath)
        except Exception as e:
            print(str(e))
        ...
        try:
            ...
            print(str(e))
            write_errlog(os.path.join(err_pdf2txt_dir, filename_only + '.log'), str('no key in .pkl files \n', e))
